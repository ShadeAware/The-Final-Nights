...
/obj/vampire_car/emergency
	...
	var/datum/looping_sound/car_emergency_siren/looping_siren

/obj/vampire_car/emergency/Initialize(mapload)
	. = ..()
	emergency_colors = list("#CC0000", "#FFFFFF")
	looping_siren = new(list(src), FALSE)
...
/datum/action/carr/emergency_response/Trigger()
	...
	to_chat(owner, span_notice("Response Code [emergency_vehicle.emergency_code] ([code_names[emergency_vehicle.emergency_code]])"))
...
/obj/vampire_car/emergency/proc/can_load_human(mob/living/carbon/human/target, mob/user)
	...
	if(length(passengers) >= max_passengers)
		to_chat(user, span_warning("There's no space left for [target] in [src]."))
		return FALSE
...
/obj/vampire_car/emergency/proc/load_human(mob/living/carbon/human/target, mob/user)
	...
	visible_message(span_notice("[user] begins loading [target] into [src]."))
	if(!do_mob(user, target, 2 SECONDS))
		to_chat(user, span_warning("You fail to load [target] into [src]."))
		return FALSE
	...
	visible_message(span_notice("[user] loads [target] into [src]."))
...
/obj/vampire_car/emergency/police/proc/store_weapon(obj/item/weapon, mob/user)
	...
	if(!weapon_type || weapon_slots[weapon_type])
		to_chat(user, span_warning("[weapon_type ? "The [weapon_type] slot is occupied" : "This weapon cannot be stored here"]!"))
		return

	visible_message(span_notice("[user] begins securing [weapon] in the weapon rack."))
	if(!do_mob(user, src, 1.5 SECONDS))
		return

	user.transferItemToLoc(weapon, src)
	weapon_slots[weapon_type] = weapon
	visible_message(span_notice("[user] secures [weapon] in the weapon rack."))
	playsound(src, DOOR_SOUND, 50, TRUE)
...
/obj/vampire_car/emergency/police/proc/retrieve_weapon(mob/user)
	...
	if(!length(available_weapons))
		to_chat(user, span_warning("The weapon rack is empty."))
		return

	var/chosen_weapon = tgui_input_list(user, "Select a weapon:", "Weapon Rack")
	if(!chosen_weapon || !user.canUseTopic(src, BE_CLOSE))
		return

	var/obj/item/selected_weapon = available_weapons[chosen_weapon]
	visible_message(span_notice("[user] begins retrieving [selected_weapon] from the weapon rack."))
	if(!do_mob(user, src, 1.5 SECONDS))
		return
	...
	visible_message(span_notice("[user] retrieves [selected_weapon] from the weapon rack."))
	playsound(src, DOOR_SOUND, 50, TRUE)
...
/obj/vampire_car/emergency/ambulance/proc/load_medical_structure(obj/structure/structure, mob/user)
	...
	if(length(occupants) && length(passengers) + length(occupants) > max_passengers)
		to_chat(user, span_warning("There's no space left for all occupants in [src]."))
		return

	visible_message(span_notice("[user] begins loading [structure] into [src]."))
	...
	visible_message(span_notice("[user] loads [structure] into [src]."))
	playsound(src, DOOR_SOUND, 50, TRUE)
...
/obj/vampire_car/emergency/ambulance/proc/unload_medical_structure(mob/user)
	...
	visible_message(span_notice("[user] begins unloading [structure_to_unload] from [src]."))
	...
	visible_message(span_notice("[user] unloads [structure_to_unload] from [src]."))
	playsound(src, DOOR_SOUND, 50, TRUE)
